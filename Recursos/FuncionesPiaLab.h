...
int minmaxInt(char itxt[], int imin, int imax){
    int iinp,ivalid=1;
    char aux[100];
    /*int input validacion, todo tiene sufijo i que se refiere a int*/
    /*no se puede tener que el caso de que el maximo es -1 ni el minimo
    porque estos se usan para decir que NO HAY maximo o minimo*/
	...
	fflush(stdin);
        scanf("%99s",aux);
        /*Se lee a un string y despues se puede leer con sscanf*/
        if(sscanf(aux,"%d",&iinp)){
            /*sscanf regresa la cantidad de coincidencias (1 o 0)
...
void solicitud_dato(int opc_dat, char *buffer){
	...
	switch(opc_dat){
		case 1:
			...
			/* Usando minmaxInt para validar la entrada */
			opc = minmaxInt("\nClave del servicio deseado: ", 1, 2);
		...
		case 4:
			...
			if(edad >= 0 && edad <= 2){
				...
				dia = minmaxInt("\nIngrese los dias del bebe (0-30): ", 0, 30);
